...
class DjediTest(TransactionTestCase):
    ...
    def settings(self, *args, **kwargs):
        with super().settings(*args, **kwargs), cio_settings():
            configure()
            yield
...
class UserMixin:
    def create_djedi_master(self):
        return User.objects.create_superuser("master", "master@djedi.io", "test")

    def create_djedi_apprentice(self):
        user = User.objects.create_user(
            "apprentice",
            email="apprentice@djedi.io",
...
class ClientTest(DjediTest, UserMixin, AssertionMixin):
    def setUp(self):
        ...
        client.login(username=master.username, password="test")  # noqa: S106
        self.client = client
