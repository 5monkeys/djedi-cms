...
class PrivateRestTest(ClientTest):
    ...
    def test_set(self):
        ...
        response = self.post(
            ...
            {"data": u"# Djedi", "data[extra]": u"foobar"},
        )
        self.assertEqual(response.status_code, 400)

        uri = "i18n://sv-se@page/title.md"
        response = self.post(
            "api", uri, {"data": u"# Djedi", "meta[message]": u"lundberg"}
        )
        self.assertEqual(response.status_code, 200)
        node = json_node(response, simple=False)
        meta = node.pop("meta")
        ...
        response = self.post(
            "api", node.uri, {"data": u"# Djedi", "meta[message]": u"Lundberg"}
        )
        node = json_node(response, simple=False)
    ...
    def test_publish(self):
        ...
        self.assertEqual(
            json_node(response),
            {"uri": "i18n://sv-se@page/title.txt#1", "content": u"Djedi"},
        )
    ...
    def test_editor(self):
        ...
        for ext in plugins:
            ...
            self.assertEqual(response.status_code, 200)
            if ext == "img":
                assert set(response.context_data.keys()) == set(
    ...
    def test_save_nested_content(self):
        ...
        self.assertEqual(response.status_code, 200)
        assert set(response.context_data.keys()) == set(
            ("THEME", "VERSION", "PLUGINS", "uri", "plugin", "forms")
        )
        assert "HTML" in response.context_data["forms"]
...
class PublicRestTest(ClientTest):
    ...
    def test_nodes(self):
        ...
        self.assertEqual(
            json_content["i18n://sv-se@rest/page/body.md"], u"<h1>Foo Bar</h1>"
        )
